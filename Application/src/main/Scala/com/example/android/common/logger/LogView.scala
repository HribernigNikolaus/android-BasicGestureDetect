...
class LogView(context: Context, attrs: AttributeSet = null, defStyle: Int = 0) extends TextView(context, attrs, defStyle) with LogNode {
  ...
  private def appendIfNotNull(source: StringBuilder, addStr: String, delimiter: String): StringBuilder = {
    var newDelimiter:String = delimiter
    if (addStr != null) {
      if (addStr.length == 0) {
        newDelimiter = ""
      }
      return source.append(addStr).append(newDelimiter)
    }
    return source
  }
